...
function minutesUntil(iso: string | null | undefined): number | null {
  ...
  return isNaN(ms) ? null : Math.max(0, ms / 60_000)
}
...
/** MiroFish-inspired composite scoring: urgency 40%, liquidity 20%, volume 20%, dislocation 20% */
export function scoreMarkets(events: RawMarket[]): ScoredMarket[] {
  ...
  for (const ev of events) {
    ...
    for (const m of markets) {
      ...
      const uScore = mins === null ? 0.1 : mins <= 5 ? 1 : mins <= 15 ? 0.95 : mins <= 60 ? 0.8 : mins <= 360 ? 0.6 : mins <= 1440 ? 0.4 : 0.15
  ...
  return flat.filter(m => m.active).sort((a, b) => b.score - a.score)
}
